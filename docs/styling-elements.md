...
- `.campfire-passage` – wrapper around rendered passages; includes `h-full` for full height and defaults to light text
...
Most classes ship with no associated styles. `.campfire-base` applies a dark gray background with light foreground text, `.campfire-passage` stretches passages to full height while keeping text light, and `.campfire-link` and `.campfire-trigger` share a button style set for convenience.
...
- `--color-primary-50` through `--color-primary-950` alias the red palette. `--color-primary` defaults to `--color-primary-600`, and `--color-primary-foreground` uses `--color-gray-50` for contrast on dark buttons.
...
- `--color-foreground` sets the default text color for file inputs and similar elements and defaults to `--color-gray-50`.
